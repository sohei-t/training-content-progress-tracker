...
- **実行**: `python3 ~/Desktop/git-worktree-agent/src/simplified_github_publisher.py .`
- **NEW v8.1**: mainブランチ → gh-pagesブランチ自動同期（GitHub Pages用）
...
- **実行**: `python3 ~/Desktop/git-worktree-agent/src/github_portfolio_publisher.py`
...
# v8.1: 自動的にgh-pagesブランチにも同期（GitHub Pages用）
```

**🆕 v8.1 gh-pages同期（自動実行）:**
```
処理フロー:
1. mainブランチにpush
2. gh-pagesブランチに自動同期（存在しなければ作成）
3. GitHub Pagesはgh-pagesブランチから配信

確認方法:
✅ https://sohei-t.github.io/ai-agent-portfolio/{slug}/ でアクセス
...
🌐 GitHub Pages:
[統合] https://sohei-t.github.io/ai-agent-portfolio/todo-app/  ← v8.1 gh-pages自動同期
